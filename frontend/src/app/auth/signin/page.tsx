...
function LoginForm() {
  ...
  const afterSignIn = safeCallbackUrl(searchParams.get("callbackUrl"));
      ...
      <div className="bg-[var(--dark-card)] border border-[var(--gold-20)] p-10 shadow-3xl">
        ...
        {error && (
          <div className="mt-6 p-4 border border-red-500/20 bg-red-500/10">
            <p className="font-mono text-[10px] uppercase text-red-500 text-center tracking-widest">
  ...
  return (
    <main className="min-h-screen bg-[var(--cream)] flex flex-col pt-[64px]">
      <div className="flex-1 flex items-center justify-center px-10 py-20">
        <Suspense fallback={<div className="font-mono text-[10px] uppercase text-[var(--muted)] animate-pulse">Initializing security protocol...</div>}>
          <LoginForm />
        </Suspense>
      </div>

      {/* Repeating Footer for Login Consistency */}
